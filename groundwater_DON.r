...
## combine similar vegetation, soil, and landuse types 
v_Veg<-values(Veg)
...
wilcox.test<-function(target,variable){
  
  data_set<-data.frame(DON=jitter(M2_train[,target]),
                       x_var=M2_train[,variable],
                       x=paste0(M2_train[,variable],"-",M2_train[,"GW"]),
                       GW=M2_train[,"GW"])
  
  keep_level<-count(data_set$x)[1][count(data_set$x)[2]>=4]
  data_set=subset(data_set,data_set$x %in% keep_level)
  data_set$x<-factor(data_set$x,levels = keep_level)
  
  pp<-pairwise.wilcox.test(data_set[,1], data_set[,3], 
  ...
  sig=data.frame(keep_level,letters=myletters$Letters,row.names = NULL)

  data_set %>% 
    ggplot(aes(x,DON))+
    geom_boxplot(aes(col=GW))+
    theme_bw()+theme(panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(),
                     legend.position = c(0.5,0.8))+
    scale_y_continuous(breaks = seq(0,6.0,1.0))
    
  print(letters)  
}


wilcox.test("DON","Soil")
wilcox.test("DON","Veg")
wilcox.test("DON","Landuse")
wilcox.test("DON","Collect_Month")
...
### soil ~ boxplot
  ...
  keep_level<-count(data_set$x)[1][count(data_set$x)[2]>=4]
  data_set=subset(data_set,data_set$x %in% keep_level)
  data_set$x<-factor(data_set$x,levels =c("10-Deep","2-Deep","13-Deep",
                                          "12-Deep","12-Shallow","11-Deep",
                                          "11-Shallow","21-Deep"))
  ...
  sig_soil=data.frame(keep_level,letters=myletters$Letters,row.names = NULL)
...
### veg~ boxplot
  ...
  data_set=subset(data_set,data_set$x %in% keep_level)
  data_set$x<-factor(data_set$x,levels =c("8-Deep","17-Deep","18-Deep",
                                          "32-Deep","1-Deep","1-Shallow",
                                          "29-Deep","29-Shallow","25-Deep"))
  
  pp<-pairwise.wilcox.test(data_set[,1], data_set[,3], 
                           p.adjust.method = "none", paired = FALSE)
  mymat<-tri.to.squ(pp$p.value)
  myletters<-multcompLetters(mymat,compare="<=",threshold=0.05,Letters=letters)
  sig_veg=data.frame(keep_level,letters=myletters$Letters,row.names = NULL)
...
## landuse~boxplot
  ...
  keep_level<-count(data_set$x)[1][count(data_set$x)[2]>=4]
  data_set=subset(data_set,data_set$x %in% keep_level)
  data_set$x<-factor(data_set$x,levels =c("1-Deep","1-Shallow","9-Deep",
                                          "3-Deep","3-Shallow","8-Deep",
                                          "8-Shallow"))
  ...
  mymat<-tri.to.squ(pp$p.value)
  myletters<-multcompLetters(mymat,compare="<=",threshold=0.05,Letters=letters)
  sig_land=data.frame(keep_level,letters=myletters$Letters,row.names = NULL)
...
### month~boxplot 
  ...
  mymat<-tri.to.squ(pp$p.value)
  myletters<-multcompLetters(mymat,compare="<=",threshold=0.05,Letters=letters)
  sig_month=data.frame(keep_level,letters=myletters$Letters,row.names = NULL)

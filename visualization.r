...
#M2_map_data$dep <- ifelse(M2_map_data$GW_depth > -10, c("shallow"), c("deep")) 
#WP2Train$dep <- ifelse(WP2Train$GW_depth > -10, c("shallow"), c("deep")) 

#M2_map_data$dep<-factor(M2_map_data$dep)
...
#M4_train$DON_k <- map1_predict_train$predicted_DON
#M4_test$DON_k <-map1_predict$predicted_DON

#M4_train$DON_rf <- h20_rf_pred_train$predict
#M4_test$DON_rf <-h20_rf_pred$predict
...
ggplot(data = map4_uncert_df, aes(y = Latitude, x = Longitude)) +
  ...
  geom_point(data=training_dataset,aes(y=training_dataset$s2,x=training_dataset$s1),size=3,
             shape=3,col="black")+
  coord_equal() +
  theme(panel.grid = element_blank(), legend.position = "right", legend.key = element_blank())+
